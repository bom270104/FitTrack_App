...
>>>>>>> 9e9779d41f31267d2fe8003572350c0b3e3e105d
        ...
        try {
          ...
          const body = `Hi ${user.fullName || "user"},\n\nĐây là lời nhắc nhở uống nước, hãy duy trì thói quen tốt cho sức khỏe của bạn!\n\n— FitTrack`;
          await sendMail({ to: user.email, subject: subj, text: body });
          await markLastSent(s.userId, new Date());
          console.log(
            `[scheduler] Sent reminder email to ${who} at ${now.toISOString()}`,
          );
        } catch (err) {
          console.error(
            `[scheduler] Failed sending reminder to ${who}:`,
            err.message || err,
          );
        }
      }
